...
admin_keyboard = ReplyKeyboardMarkup(
    [
        [KeyboardButton("📊 آمار کانفیگ‌ها")],
        [KeyboardButton("🗑️ حذف کانفیگ‌های منفی")],
        [KeyboardButton("🔙 بازگشت")]
    ],
    resize_keyboard=False
)
...
# ---------- ارسال کانفیگ با دکمه رأی ----------
async def send_config(update, cfg_id, cfg, user_id):
    keyboard_vote = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("👍 مثبت", callback_data=f"vote_up:{cfg_id}"),
                InlineKeyboardButton("👎 منفی", callback_data=f"vote_down:{cfg_id}")
            ]
        ]
    )
    await update.message.reply_text(
...
async def vote_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    cfg_id = int(data.split(":")[1])
    vote_type = data.split(":")[0]

    cur.execute("SELECT vote FROM votes WHERE user_id=? AND config_id=?", (user_id, cfg_id))
    row = cur.fetchone()
    if row:
        await query.edit_message_text("⚠️ شما قبلاً رأی داده‌اید.")
        return

    if vote_type == "vote_up":
        cur.execute("INSERT INTO votes (user_id, config_id, vote) VALUES (?,?,?)", (user_id, cfg_id, 1))
        cur.execute("UPDATE configs SET votes_up = votes_up + 1 WHERE id=?", (cfg_id,))
    elif vote_type == "vote_down":
        cur.execute("INSERT INTO votes (user_id, config_id, vote) VALUES (?,?,?)", (user_id, cfg_id, -1))
        cur.execute("UPDATE configs SET votes_down = votes_down + 1 WHERE id=?", (cfg_id,))
    db.commit()
    await query.edit_message_text("✅ رأی شما ثبت شد.")
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if user_id != ADMIN_ID:
        await update.message.reply_text("⚠️ دسترسی ادمین ندارید.")
        return
    try:
        await update.message.delete()
    except:
...
async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------- دستورات ادمین ----------
    if user_id == ADMIN_ID:
        if text == "📊 آمار کانفیگ‌ها":
            cur.execute("SELECT COUNT(*) FROM configs")
            count = cur.fetchone()[0]
            await update.message.reply_text(f"📌 تعداد کل کانفیگ‌ها: {count}", reply_markup=admin_keyboard)
            return
        elif text == "🗑️ حذف کانفیگ‌های منفی":
            delete_negative_configs()
            await update.message.reply_text("✅ کانفیگ‌های با رأی منفی بالا حذف شدند.", reply_markup=admin_keyboard)
            return
        elif text == "🔙 بازگشت" and update.message.reply_text:
    ...
    if text.startswith("📥 دریافت کانفیگ"):
        ...
        for cfg_id, cfg in selected:
            await send_config(update, cfg_id, cfg, user_id)
        return

    if text == "📝 راهنما":
        guide_text = (
            "🎓 راهنمای استفاده از ربات:\n\n"
            "➕ ارسال کانفیگ: برای ثبت کانفیگ‌های خودتان استفاده کنید.\n"
            "📥 دریافت کانفیگ: کانفیگ‌های امن و رایگان دریافت کنید.\n"
            "👍 / 👎 رأی‌گیری: پس از دریافت هر کانفیگ، دو دکمه 👍 و 👎 نمایش داده می‌شود. "
            "با رأی مثبت، کیفیت کانفیگ‌ها حفظ می‌شود و با رأی منفی، کانفیگ‌هایی که کیفیت پایینی دارند حذف خواهند شد.\n"
            "🔙 بازگشت: برای برگشت به منوی اصلی.\n\n"
            "🌐 اینترنت آزاد و رایگان در اختیار شماست!\n"
            "💡 لطفاً از گزینه‌ها به ترتیب و مطابق راهنما استفاده کنید."
        )
        await update.message.reply_text(guide_text, reply_markup=keyboard)
        return
...
app.add_handler(CommandHandler("admin", admin_panel))  # /admin برای ادمین
